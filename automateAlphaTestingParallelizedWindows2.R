...
funcRun <- function(offset=0, subtest_folder = "", subresult_folder = "", bln.corr = FALSE){
  ...
  driver1 <- rsDriver(browser=c("chrome"), port = 4557L, chromever="73.0.3683.68")
  driver2 <- rsDriver(browser=c("chrome"), port = 4558L, chromever="73.0.3683.68")
  driver3 <- rsDriver(browser=c("chrome"), port = 4559L, chromever="73.0.3683.68")
  ...
  alpha_list <- alpha_list[(ceiling(length(alpha_list)/4)+1):ceiling(length(alpha_list)/2)]
