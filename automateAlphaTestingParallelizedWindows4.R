...
username = "pil2104@columbia.edu"
password = "columbiadsiers2019"
...
funcRun <- function(offset=0, subtest_folder = "", subresult_folder = "", bln.corr = FALSE, bln.all = FALSE){
  
  driver1 <- rsDriver(browser=c("chrome"), port = 4453L, chromever="73.0.3683.68")
  driver2 <- rsDriver(browser=c("chrome"), port = 4454L, chromever="73.0.3683.68")
  driver3 <- rsDriver(browser=c("chrome"), port = 4455L, chromever="73.0.3683.68")
  ...
  if (!bln.all){
    alpha_list <- alpha_list[(ceiling(length(alpha_list)*0.75)+1):length(alpha_list)]
  }
...
funcRun(offset=-1, bln.all = FALSE)
